...
const FEAASScripts = (): JSX.Element => {
  ...
  const shouldOptimize = (src: string) => {
    if (src.startsWith("http")) {
      ...
      return (
        ...
        remotePatterns.some((pattern) => pattern.protocol === url.protocol.slice(0, -1) && new RegExp("^" + convertToRegex(pattern.hostname) + "$").test(url.hostname))
      );
    }
    return true;
  };
  ...
  FEAAS.setElementImplementation("img", (attributes: Record<string, string>) => {
    ...
    return <Image height={1920} width={1200} unoptimized={!shouldOptimize(src)} src={src} alt={alt} {...imgAttributes} />;
  });
