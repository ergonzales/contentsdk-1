...
export const Default = (props: LinkListProps): JSX.Element => {
  ...
  if (datasource) {
    const list = datasource.children.results
      ...
      .map((element: ResultsFieldLink, key: number) => <LinkListItem index={key} key={`${key}${element.field.link}`} total={datasource.children.results.length} field={element.field.link} />);
