...
const BYOCInit = (): JSX.Element | null => {
  const providerState = React.useContext(SitecoreProviderReactContext);
  const pageState = providerState.page?.layout?.sitecore?.context?.pageState;
  ...
  FEAAS.setContextProperties({
    ...
    siteName: providerState.page?.siteName || scConfig.defaultSite,
    eventsSDK: Events,
  });
